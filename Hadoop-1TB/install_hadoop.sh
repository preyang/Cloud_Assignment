...
##  Install the java in the instance.  
...
#check java version using below command
sudo java -version


## generate ssh key and put in into authorized_keys folder.
ssh-keygen -t rsa -P ""

cat $HOME/.ssh/id_rsa.pub >> $HOME/.ssh/authorized_keys

##install hadoop using below command.. Untar it in and change some property as need in program.
cd /usr/local
...
sudo rm -r hadoop
##make new folder name hadoop and copy hadoop 2.7.1 to hadoop folder.
sudo mkdir hadoop
sudo mv hadoop-2.7.1/* hadoop/


## set enviornment variables used for java and hadoop in bashrc file.
sudo cat >> ~/.bashrc <<EOF
...
##below command is used for update the bsahrc changes that we made.
source ~/.bashrc

cd /usr/local/hadoop/etc/hadoop

# Now update hadoop configuration file  
...
##give permission to this file and Change in this xml file to add tmp directory. We are setting /mnt/raid/tmp part as we are working on mounted disks.
...
##Give permission to this file and set reducers as per needed in program.
...
##give permission to this file.set disk utilization thresold in this xml file.
...
##give permission to this file.set block size in this xml file.
...
## go back to the root folder in directory.
cd ~
